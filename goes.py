...
def read_goes(sdate,edate=None,sat_nr="15",is_high_res=False):
    ...
    if edate is None: edate = sdate + dt.timedelta(days=31)
    ...
    if len(f_paths) > 0:
	data_dict = __extract_data(f_paths, data_dict, int(sat_nr), sdate, edate, is_high_res)
    else:
