...
##
# Build dataframe data for deterministic regressor model with goes R,B data 
##
def load_data_with_goes_RB_for_deterministic_reg(mI=1):
    params = ["Bx_m","By_m","Bz_m","V_m","Vx_m","Vy_m","Vz_m","PR_d_m","T_m","P_dyn_m","E_m","beta_m",
            "Ma_m","sdates","Kp","_kp_lt","delay_time","_dkp","_dkp_lt"]
    headers = ["B_x","B_T","sin_tc","V","n","T",
            "P_dyn","beta","M_A","Date_WS","K_P","K_P_LT",
            "Date_FC","K_P_delay","K_P_LT_delay"]
    delay = 3*mI
    fname = "data_store/omni_goes_RB_3h_%d.csv"%delay
    if not os.path.exists(fname):
...
def load_data_with_goes_RB_for_deterministic_cls(th=4.5, mI=1):
    params = ["Bx_m","By_m","Bz_m","V_m","Vx_m","Vy_m","Vz_m","PR_d_m","T_m","P_dyn_m","E_m","beta_m",
            "Ma_m","sdates","Kp","_kp_lt","delay_time","_dkp","_dkp_lt"]
    headers = ["B_x","B_T","sin_tc","V","n","T",
            "P_dyn","beta","M_A","Date_WS","K_P","K_P_LT",
            "Date_FC","K_P_delay","K_P_LT_delay"]
    delay = 3*mI
    fname = "data_store/omni_goes_RB_3h_%d_cls.csv"%delay
    if not os.path.exists(fname):
        _g = pd.read_csv("data_store/goes_prep_3h.csv")
        _g.date = pd.to_datetime(_g.date)
        _o = read_omni_data()
        ...
        for I,rec in _o.iterrows():
            ...
            now_g = _g[_g.date == (now - dt.timedelta(hours=12))]
            if len(now_g) == 0  and len(_goes_R) > 0:
                print _goes_R[-1], _goes_B[-1]
                _goes_R.append(_goes_R[-1])
                _goes_B.append(_goes_B[-1])
            elif len(now_g) > 0:
                print now_g.R.tolist()[0], now_g.B.tolist()[0]
                _goes_R.append(now_g.R.tolist()[0])
                _goes_B.append(now_g.B.tolist()[0])
            else: 
                _goes_R.append(1.0)
        ...
        _o.Date_WS = pd.to_datetime(_o.Date_WS)
        pass
    _xparams = ["B_x","B_T","sin_tc","V","n","T","P_dyn","beta","M_A","K_P_LT","goes_R","goes_B"]
    _xparams = ["B_x","B_T","sin_tc","V","n","T","P_dyn","beta","M_A","K_P_LT"]
    _yparam = ["stormL"]
